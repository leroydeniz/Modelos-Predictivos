...
Arrival_Delay = 1.239 + 0.0039 * Distance_i
...
```{r }
ad_model2 <- lm(formula=Arrival_Delay ~ Distance+Departure_Delay, df)
summary(ad_model2)
```
...
```{r }
ggplot(df, aes(x = Arrival_Delay, y=Departure_Delay)) +
  geom_point() +
  stat_smooth( method = "lm", se=FALSE)
```
...
```{r }
ModelTmp <- lm(formula=Arrival_Delay ~ Distance+Departure_Delay+Service+Food_drink+satisfaction+Customer_Type, df)
summary(ModelTmp)
```

\vspace{0.5cm}

Las variables food y customer_type se van porque no son significantes
...
```{r }
ModelF <- lm(formula=Arrival_Delay ~ Distance+Departure_Delay+satisfaction, df)
summary(ModelF)
```
...
predict(ModelF, data.frame("satisfaction"=x_satisfaction,"Distance"=x_distance, "Departure_Delay"=x_departure_delay),type ="response")
...
Los valores del _GVIF_ (versión generalizada del coeficiente VIF), en la tercer columna de este nuevo modelo, presentan diferencias con respecto al modelo de regresión lineal del apartado anterior, puesto que aquí sí existe el problema de la colinealidad con las variables _Departure_Delay_ y _Arribal_Delay_, cuyos valores no son tan moderados en comparación con las demás. Al tomar como referencia los valores de esta tercera columna, se está mostrnado cuántas veces la variable es más grande que si era ortogonal a las otras.

El nuevo modelo entonces no debería incluir las variables _Departure_Delay_ y _Arribal_Delay_.
